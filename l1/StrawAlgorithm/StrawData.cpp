...
inline int StrawData::getSrbID(uint8_t chamber, uint8_t view, uint8_t halfView) {
...
void StrawData::readDataWord(int hitNum) {
	...
	int tempFineTime = 0;
	for (int i = 12; i > 7; i--) {
		if (tempBinary[i] == 1) tempFineTime += (int) pow(2,12-i);
	}
	m_fineTime = decodeFineTime(tempFineTime);
	for (int i = 5; i > -3; i--) {
		if (((i < 0)&&(tempBinary[16 + i] == 1))||((i >= 0)&&(tempBinary[i] == 1))) {
