...
// getter functions
	inline uint8_t getL0TriggerDecision() {
		return m_strawDataHdr.triggerTypeWord;
	}
	inline uint8_t getChamber() {
		return m_strawDataHdr.chamber;
	}
	inline uint8_t getView() {
		return m_strawDataHdr.view;
	}
	inline uint8_t getHalfView() {
		return m_strawDataHdr.halfView;
	}
	inline uint16_t getPacketLength() {
		return m_strawDataHdr.packetLength;
	}
	inline uint16_t getCoarseTime() {
		return m_coarseTime;
	}
	inline uint8_t getError() {
		return m_error;
	}
	inline uint8_t getStrawID() {
		return m_strawID;
	}
	inline uint8_t getPlaneID() {
		return m_planeID;
	}
	inline uint8_t getEdgeType() {
		return m_edgeType;
	}
	inline double getFineTime() {
		return m_fineTime;
	}
