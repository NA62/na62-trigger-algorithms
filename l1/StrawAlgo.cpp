...
//				printf ("inside loop: j=%d \n",j);
				if ((strawPrecluster_[chamberID][viewID][halfviewID][j].plane
					...
					if ((edgeIsLeading[iEdge])
							&& (leading
									< strawPrecluster_[chamberID][viewID][halfviewID][j].leading
									|| strawPrecluster_[chamberID][viewID][halfviewID][j].leading
											<= -10)
						...
						if (leading < 1)
							...
							wireDistance = strawDigiMan_.rTDependenceData(
									leading / 1000);
...
//				printf ("New straw:  ");
				...
				strawPrecluster_[chamberID][viewID][halfviewID][nStrawPreclusters[chamberID][viewID][halfviewID]].setStraw(
...
//				strawPrecluster_[i][g][0][j].printStraw();
				...
				if (((strawPrecluster_[i][g][0][j].leading
						< (m1leadtrail * strawPrecluster_[i][g][0][j].trailing
								+ q1leadtrail)
						&& strawPrecluster_[i][g][0][j].leading
								> (m2leadtrail
										* strawPrecluster_[i][g][0][j].trailing
										+ q2leadtrail)
						&& (strawPrecluster_[i][g][0][j].trailing
								> cutlowtrailing
								&& strawPrecluster_[i][g][0][j].trailing
						...
						&& (strawPrecluster_[i][g][0][j].leading > cutlowleading
								&& strawPrecluster_[i][g][0][j].leading
										< cuthighleading)) {
					for (int h = j + 1;
							h < nStrawPreclusters[i][g][0]
									&& !strawPrecluster_[i][g][0][j].used; h++) //prendo un hit appertenente al piano 2 o 3
							{
//						printf("hit 2 (%d): ", h);
//						strawPrecluster_[i][g][0][h].printStraw();

						if (((strawPrecluster_[i][g][0][h].leading
								< (m1leadtrail
...
//							printf("distance = %f  \n", temp_distance);
							if (temp_distance < 9 && temp_distance > -9
								...
								if (temp_distance > 0) {
									positionh =
											strawPrecluster_[i][g][0][h].position
													+ strawPrecluster_[i][g][0][h].wiredistance;
									positionj =
											strawPrecluster_[i][g][0][j].position
								...
								if (strawPrecluster_[i][g][0][h].trailing > -200
									...
									if (strawPrecluster_[i][g][0][h].trailing
											<= -200)
										trailing_cluster =
												strawPrecluster_[i][g][0][j].trailing;
									else
...
//										deltadistance_triplets);
								...
								if (deltadistance_triplets > hit3low
									...
									strawCluster_[i][g][nStrawClusters[i][g]].setCluster(
											strawPrecluster_[i][g][0][h].chamber,
											strawPrecluster_[i][g][0][h].view,
...
//									strawPrecluster_[i][g][0][h].printStraw();
									...
									for (int l = 0;
										...
										temp_distance =
												fabs(
														meandistance
																- strawPrecluster_[i][g][1][l].position);
										if (temp_distance < 5
												&& !strawPrecluster_[i][g][1][l].used)
											strawPrecluster_[i][g][1][l].used =
													1;
									}
								}
...
//				strawPrecluster_[i][g][1][j].printStraw();
				...
				if (((strawPrecluster_[i][g][1][j].leading
						< (m1leadtrail * strawPrecluster_[i][g][1][j].trailing
								+ q1leadtrail)
						&& strawPrecluster_[i][g][1][j].leading
								> (m2leadtrail
										* strawPrecluster_[i][g][1][j].trailing
										+ q2leadtrail)
						&& (strawPrecluster_[i][g][1][j].trailing
								> cutlowtrailing
								&& strawPrecluster_[i][g][1][j].trailing
						...
						&& (strawPrecluster_[i][g][1][j].leading > cutlowleading
								&& strawPrecluster_[i][g][1][j].leading
										< cuthighleading)) {
					for (int h = j + 1;
							h < nStrawPreclusters[i][g][1]
									&& !strawPrecluster_[i][g][1][j].used; h++) //prendo un hit appertenente al piano 2 o 3
							{
//						printf("hit 2 (%d): ", h);
//						strawPrecluster_[i][g][1][h].printStraw();
						if (((strawPrecluster_[i][g][1][h].leading
								< (m1leadtrail
...
//							printf("distance = %f \n", temp_distance);
							...
							if (temp_distance < 9 && temp_distance > -9
								...
								if (temp_distance > 0) {
									positionh =
											strawPrecluster_[i][g][1][h].position
													+ strawPrecluster_[i][g][1][h].wiredistance;
									positionj =
											strawPrecluster_[i][g][1][j].position
								...
								if (strawPrecluster_[i][g][1][h].trailing > -200
									...
									if (strawPrecluster_[i][g][1][h].trailing
											<= -200)
										trailing_cluster =
												strawPrecluster_[i][g][1][j].trailing;
									else
...
//										deltadistance_triplets);
								...
								if (deltadistance_triplets > hit3low
										&& deltadistance_triplets < hit3high) {
									strawCluster_[i][g][nStrawClusters[i][g]].setCluster(
											strawPrecluster_[i][g][1][h].chamber,
											strawPrecluster_[i][g][1][h].view,
											meandistance, trailing_cluster,
											deltadistance_triplets, 0);
...
//									strawPrecluster_[i][g][1][h].printStraw();
									...
									for (int l = 0;
											l < nStrawPreclusters[i][g][0]; l++) //cerco il 3 hit negli altri 2 piani
											{
										temp_distance =
												fabs(
														meandistance
																- strawPrecluster_[i][g][0][l].position);
										if (temp_distance < 5
												&& !strawPrecluster_[i][g][0][l].used)
											strawPrecluster_[i][g][0][l].used =
													1;
									}
								}
							...
							if (temp_distance < 9
								...
								case (0):
									if (!strawPrecluster_[i][g][0][h].plane) {
										...
										meandistance = (positionh + positionj)
												/ 2;
		...
		for (int a = 0; a < nStrawClusters[i][3]; a++) //clusters [a] is inside y
			...
			for (int b = 0; b < nStrawClusters[i][2]; b++)  //we loop on x view
				...
				for (int c = 0; c < nStrawClusters[i][1]; c++) //we loop on u  views
						{
					coordinate_temp = (strawCluster_[i][3][a].coordinate
							+ strawCluster_[i][2][b].coordinate) / sq2;
					viewdistance = fabs(
							strawCluster_[i][1][c].coordinate
									- coordinate_temp);
							...
							if (viewdistance4 < cutcluster) {
								...
								if (viewdistance < viewdistance4)
									strawPointTemp_[i][nStrawPointsTemp[i]].setPoint(
											...
											(strawCluster_[i][3][a].trailing
													+ strawCluster_[i][2][b].trailing
...
//					{
				for (int c = 0; c < nStrawClusters[i][1]; c++) //we loop on u  views
					...
					if ((strawCluster_[i][1][c].used != 1
							&& strawCluster_[i][2][b].used != 1)
							or (strawCluster_[i][1][c].used != 1
									&& strawCluster_[i][3][a].used != 1)
							or (strawCluster_[i][2][b].used != 1
									&& strawCluster_[i][3][a].used != 1)) {
						coordinate_temp = (strawCluster_[i][3][a].coordinate
								+ strawCluster_[i][2][b].coordinate) / sq2;
						viewdistance = fabs(
...
//							strawPointTemp_[i][nStrawPointsTemp[i]].printPoint();
				...
				for (int c = 0; c < nStrawClusters[i][0]; c++) //we loop on v views
					...
					if ((strawCluster_[i][0][c].used != 1
						...
						viewdistance = fabs(
								strawCluster_[i][0][c].coordinate
										- coordinate_temp);
...
//				LOG_INFO << nStrawClusters[i][2] << ENDL;
				for (int c = 0; c < nStrawClusters[i][2]; c++) //x
					...
					if ((strawCluster_[i][0][a].used != 1
							&& strawCluster_[i][1][b].used != 1)
							or (strawCluster_[i][0][a].used != 1
									&& strawCluster_[i][2][c].used != 1)
							or (strawCluster_[i][1][b].used != 1
									&& strawCluster_[i][2][c].used != 1)) {
						coordinate_temp = (strawCluster_[i][0][a].coordinate
								+ strawCluster_[i][1][b].coordinate) / sq2;
						viewdistance = fabs(
								strawCluster_[i][2][c].coordinate
										- coordinate_temp);
						...
						if (viewdistance < cutcluster) {
							...
							strawPointTemp_[i][nStrawPointsTemp[i]].setPoint(
									fChamberZPosition[i], xtemp, ytemp,
									(strawCluster_[i][0][a].trailing
											+ strawCluster_[i][1][b].trailing
...
//				LOG_INFO << nStrawClusters[i][3] << ENDL;
				for (int c = 0; c < nStrawClusters[i][3]; c++) //y
					...
					if ((strawCluster_[i][0][a].used != 1
							&& strawCluster_[i][1][b].used != 1)
							or (strawCluster_[i][0][a].used != 1
									&& strawCluster_[i][3][c].used != 1)
							or (strawCluster_[i][1][b].used != 1
									&& strawCluster_[i][3][c].used != 1)) {
						coordinate_temp = (strawCluster_[i][1][b].coordinate
								- strawCluster_[i][0][a].coordinate) / sq2;
						viewdistance = fabs(
								strawCluster_[i][3][c].coordinate
										- coordinate_temp);
...
//							strawPointTemp_[i][nStrawPointsTemp[i]].printPoint();
		...
		for (int a = 0; a < nStrawClusters[i][0]; a++)  //v
			...
			for (int b = 0; b < nStrawClusters[i][1]; b++) //u (v,u)
					{
				if (strawCluster_[i][1][b].used == 0
						&& strawCluster_[i][0][a].used == 0) {
					coordinate[0] = (strawCluster_[i][0][a].coordinate
							+ strawCluster_[i][1][b].coordinate) / sq2;
					coordinate[1] = (strawCluster_[i][1][b].coordinate
							- strawCluster_[i][0][a].coordinate) / sq2;

					if (strawAcceptance(i, coordinate, 12) == 1) {
						strawPointTemp_[i][nStrawPointsTemp[i]].setPoint(
								fChamberZPosition[i], coordinate[0],
								coordinate[1],
								(strawCluster_[i][0][a].trailing
										+ strawCluster_[i][1][b].trailing) / 2,
								0, 2, 0);
						nStrawPointsTemp[i]++;
...
//								nStrawPointsTemp[i], i, a, b);
			...
			for (int b = 0; b < nStrawClusters[i][2]; b++) //x (v,x)
				...
				if (strawCluster_[i][2][b].used == 0
					...
					coordinate[1] = strawCluster_[i][2][b].coordinate
					...
					if (strawAcceptance(i, coordinate, 12) == 1) {
						strawPointTemp_[i][nStrawPointsTemp[i]].setPoint(
								...
								coordinate[1],
								(strawCluster_[i][0][a].trailing
										+ strawCluster_[i][2][b].trailing) / 2,
								0, 2, 0);
						nStrawPointsTemp[i]++;
				...
				if (strawCluster_[i][3][b].used == 0
					...
					if (strawAcceptance(i, coordinate, 12) == 1) {
						strawPointTemp_[i][nStrawPointsTemp[i]].setPoint(
								...
								coordinate[1],
								(strawCluster_[i][0][a].trailing
										+ strawCluster_[i][3][b].trailing) / 2,
								0, 2, 0);
						nStrawPointsTemp[i]++;
...
//								pointTemp.setPoint(strawPointTemp_[i][k].z,strawPointTemp_[i][k].x,strawPointTemp_[i][k].y,strawPointTemp_[i][k].trailing,strawPointTemp_[i][k].viewDistance,strawPointTemp_[i][k].nViews,strawPointTemp_[i][k].used);
				...
				strawPointTempbis_[i][nStrawPointsTempbis[i]].clonePoint(
						pointTemp);
				nStrawPointsTempbis[i]++;
			}
...
//								pointTemp.setPoint(strawPointTemp_[i][k].z,strawPointTemp_[i][k].x,strawPointTemp_[i][k].y,strawPointTemp_[i][k].trailing,strawPointTemp_[i][k].viewDistance,strawPointTemp_[i][k].nViews,strawPointTemp_[i][k].used);
		...
		for (int j = 0; j < nStrawPointsTempbis[i]; j++) //loop over point
				{
			if (strawPointTempbis_[i][j].nViews == 4
				...
				for (int k = j + 1; k < nStrawPointsTempbis[i]; k++) //loop over point
						{
					if (strawPointTempbis_[i][k].nViews == 4
							&& !strawPointTempbis_[i][k].used) {
						point_dx = fabs(
...
//				pointTemp.setPoint(strawPointTempbis_[i][j].z,strawPointTempbis_[i][j].x,strawPointTempbis_[i][j].y,strawPointTempbis_[i][j].trailing,strawPointTempbis_[i][j].viewDistance,strawPointTempbis_[i][j].nViews,strawPointTempbis_[i][j].used);
				...
				for (int k = j + 1; k < nStrawPointsTempbis[i]; k++) //loop over point
						{
					if (strawPointTempbis_[i][k].nViews == 3
							&& !strawPointTempbis_[i][k].used) {
						point_dx = fabs(
								strawPointTempbis_[i][j].x
										- strawPointTempbis_[i][k].x);
...
//								pointTemp.setPoint(strawPointTempbis_[i][k].z,strawPointTempbis_[i][k].x,strawPointTempbis_[i][k].y,strawPointTempbis_[i][k].trailing,strawPointTempbis_[i][k].viewDistance,strawPointTempbis_[i][k].nViews,strawPointTempbis_[i][k].used);
			...
			if (strawPointTempbis_[i][j].nViews == 2) {
				strawPointFinal_[i][nStrawPointsFinal[i]].clonePoint(
						strawPointTempbis_[i][j]);
				nStrawPointsFinal[i]++;
			}
		}
	}
...
int StrawAlgo::strawAcceptance(int n, double *coor, int zone) {
	...
	double StrawRadius = StrawInnerRadius + 2 * CopperThickness + MylarThickness
			+ GoldThickness;
	...
	for (int jView = 0; jView < 4; jView++) {
		...
		if (((posView > fHoleChamberMax[n][jView]
				&& posView < 0.5 * fViewPlaneTransverseSize)
				|| (posView < fHoleChamberMin[n][jView]
						&& posView > -0.5 * fViewPlaneTransverseSize))
	...
	case 2:  // At least 2 views
		if ((Vx && Vy) || (Vx && Vu) || (Vx && Vv) || (Vy && Vu) || (Vy && Vv)
				|| (Vu && Vv))
			return 1;
		return 0;
	case 3:  // At least 3 views
		if ((Vx && Vy && Vu) || (Vx && Vy && Vv) || (Vx && Vu && Vv)
				|| (Vy && Vu && Vv))
			return 1;
		return 0;
